     ...
     const map = (mapContext) => {
         try {
             ...
             if (Number(contactType) == 4 && clienAlliance == 1) {
                 ...
                 let initialFormattedDateString = infoCustomer.values.datecreated;
                 ...
                 //Revisamos que sea está semana por ejemplo lunes, que no exista una oportunidad previamente creada y que tenga un tipo de servicio
                 if (week.includes(getWeek) && !existOP && !!typeService) {
                     // //log.debug('infoCustomer', infoCustomer)
                     log.debug('es la semana que toca', existOP);
                         ...
                         case '1':
                             ...
                             if (!!infoCustomer.values.custentity_ptg_articulo_frecuente) {
                                 ...
                                 if (!!itemContent['custitem_ptg_capacidadcilindro_'] && Number(priceZoneValue) > 0) {
                                     ...
                                     log.debug('se creo cilindro programado', id)
                                 }
                 ...
                 let typeService = infoCustomer.values.custentity_ptg_tipodeservicio_;
                 //Validamos que sea hoy o mañana , que no tenga un op creada y configurado el tipo de servicio
                 //if (week.includes(getWeek) || week.includes(nextWeek) && existOP === false && !!typeService) {
                 if (!existOP && !!typeService && (week.includes(getWeek) || week.includes(nextWeek))) {
                     //log.debug('infoCustomer', infoCustomer)
                     log.debug('es la semana que toca avisar', existOP);
                     ...
                     switch (typeService.value) {
                         ...
                         case '2':
                             //Similar a cilindro pero con estacionario, diferencia que el estacionario no tiene una capacidad y es por litros
